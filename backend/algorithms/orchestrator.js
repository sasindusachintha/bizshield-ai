...
function processIdeas(ideas) {
  ...
  const ideasWithScores = ideas.map((idea, idx) => {
    ...
    const validScore = Number.isFinite(rawScore) ? Math.min(10, Math.max(1, rawScore)) : 5;
    return {
      id: idea.id || idx + 1,
      name: idea.name || "",
      ...
      score: validScore, // Falls back to 5 only if the AI truly omitted/mangled a score
    };
  });
