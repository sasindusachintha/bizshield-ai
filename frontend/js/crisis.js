...
function renderFinalSummary({ conflict, finalRiskScore, finalPlan, crisisInput }) {
    ...
    const conflictBlock = conflict.highConflict
        ? `
            <div class="alert alert-danger conflict-alert d-flex align-items-center gap-3 mt-4">
                ...
                <span>
                    <strong>HIGH CONFLICT detected</strong> - ${escapeHTML(conflict.disagreementBetween[0])} and
        ...
        : `
            <div class="alert alert-success d-flex align-items-center gap-3 mt-4">
                ...
                <span>The boardroom reached reasonable alignment - no high-conflict override was required.</span>
            </div>`;
    ...
    const summaryHTML = `
        ...
        <div class="mt-4 result-reveal">
            <h5 class="mb-3 fw-semibold"><i class="fas fa-list-check text-danger me-2"></i>Recovery Plan - Step by Step</h5>
            ${stepsHTML}
        </div>
...
crisisBtn.addEventListener('click', async (event) => {
