...
function normalizePlanContent(rawPlan = {}, idea = "", language = "en") {
  ...
  return {
    ...
    weekly_progress_template: rawPlan.weekly_progress_template || null,
...
function normalizeMarketingContent(rawContent = {}, idea = "", language = "en") {
  ...
  return {
    ...
    ad_copies: Array.isArray(rawContent.ad_copies) && rawContent.ad_copies.length
      ? rawContent.ad_copies
      : socialPosts.slice(0, 3).map((post) => ({ headline: post.hook, body: post.body, cta: post.cta })),
...
`;
    ...
    const aiResponse = await callGroqWithContext(prompt, context);
...
- Each post must include hook, body, CTA, platform, and 5-10 hashtags.
...
Return ONLY this JSON format:
{
  ...
  "instagram_posts": [
    { "platform": "Instagram", "hook": "Hook line", "body": "Content body", "cta": "Call to action", "caption": "Engaging caption", "hashtags": ["#tag1", "#tag2", "#tag3", "#tag4", "#tag5"] }
  ],
  "ad_copies": [
