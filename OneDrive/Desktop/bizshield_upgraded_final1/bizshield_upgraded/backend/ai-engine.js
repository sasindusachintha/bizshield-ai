...
async function callGroqWithContext(prompt, context = {}) {
  const { user, projects = [], recentActivity = [], stage = "startup", currency = "USD", language = "en" } = context;
...
`;
  ...
  return callGroq(systemContext + securityContext + prompt, { responseLanguage });
}
