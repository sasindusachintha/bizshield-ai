...
btn.addEventListener('click', async () => {
    ...
    btn.disabled = true;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Creating kitâ€¦';
    showLoading(boardContent, 'Crafting your marketing content kitâ€¦');

    btn.innerHTML = `<span class="spinner-border spinner-border-sm me-2"></span>${t('Creating kitâ€¦')}`;
    showLoading(boardContent, t('Crafting your marketing content kitâ€¦'));

    try {
        const data = await callAPI('/marketing-content', { idea });
        console.log('ðŸ“¢ MARKETING-CONTENT RESPONSE:', data);
        renderMarketing(data);
    } catch (err) {
        console.error('ðŸ’¥ ERROR in marketing-content:', err);
...
function renderMarketing(d) {
    ...
    console.log('ðŸ›¡ï¸ SAFE MARKETING DATA:', safeData);
    d = safeData;
